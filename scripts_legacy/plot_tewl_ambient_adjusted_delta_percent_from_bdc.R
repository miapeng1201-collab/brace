...
output_file <- file.path(tewl_dir, "TEWL_ambient_adjusted_delta_percent_from_BDC_by_group_R.png")
...
source_df <- df[!is.na(df$stage_plot) & !is.na(df$tewl_ambient_adjusted), ]
...
baseline <- stage_values[stage_values$stage_plot == "BDC", c("group", "subject", "tewl_ambient_adjusted")]
names(baseline)[names(baseline) == "tewl_ambient_adjusted"] <- "BDC_adjusted"

plot_df <- merge(stage_values, baseline, by = c("group", "subject"), all.x = TRUE)
plot_df$delta_percent_from_BDC <- (plot_df$tewl_ambient_adjusted - plot_df$BDC_adjusted) / plot_df$BDC_adjusted * 100
plot_df <- plot_df[!is.na(plot_df$stage_plot) & !is.na(plot_df$delta_percent_from_BDC), ]
...
names(summary_df) <- c("group", "stage_plot", "mean_delta_percent", "se", "n")
...
p <- ggplot() +
  ...
  geom_errorbar(
    ...
    aes(
      x = stage_plot,
      ymin = mean_delta_percent - se,
      ymax = mean_delta_percent + se,
      color = group
    ),
    width = 0.18,
    linewidth = 0.7,
  ...
  geom_line(
    data = summary_df,
    aes(x = stage_plot, y = mean_delta_percent, group = group, color = group),
    linewidth = 1.15,
    na.rm = TRUE
  ) +
  geom_point(
    data = summary_df,
    aes(x = stage_plot, y = mean_delta_percent, color = group),
    size = 2.4,
    na.rm = TRUE
  ) +
  ...
  labs(
    x = NULL,
    y = "Change from adjusted BDC baseline (%)",
    color = "Group"
  ) +
