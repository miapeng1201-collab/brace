...
output_combined <- file.path(tewl_dir, "TEWL_vs_ambient_temperature_binned_three_phases_two_groups_R.png")

phase_outputs <- c(
  BDC = file.path(tewl_dir, "TEWL_vs_ambient_temperature_binned_BDC_two_groups_R.png"),
  `HDT1-19` = file.path(tewl_dir, "TEWL_vs_ambient_temperature_binned_HDT1_19_two_groups_R.png"),
  `HDT37-55` = file.path(tewl_dir, "TEWL_vs_ambient_temperature_binned_HDT37_55_two_groups_R.png")
)
...
fit_pvalues <- function(data) {
  data <- data[complete.cases(data[, c("subject", "final_clean_tewl", "ambient_temperature", "group_cm")]), ]
  ...
  model <- lmer(
    final_clean_tewl ~ ambient_temperature * group_cm + (1 | subject),
    data = data,
    REML = FALSE
  )
  a <- as.data.frame(anova(model, type = 3))
  out["temp"] <- a[rownames(a) == "ambient_temperature", "Pr(>F)"]
  out["group"] <- a[rownames(a) == "group_cm", "Pr(>F)"]
  out["temp_group"] <- a[rownames(a) == "ambient_temperature:group_cm", "Pr(>F)"]
  out
}

make_summary <- function(data, bin_width = 1) {
  data$ambient_bin <- round(data$ambient_temperature / bin_width) * bin_width
  data <- data[complete.cases(data[, c("final_clean_tewl", "ambient_bin", "group_cm")]), ]

  summary_df <- aggregate(
    final_clean_tewl ~ phase + ambient_bin + group_cm,
    data = data,
    FUN = function(x) c(mean = mean(x), sd = sd(x), n = length(x))
  )
  unpacked <- do.call(data.frame, summary_df)
  names(unpacked) <- c("phase", "ambient_bin", "group", "mean", "sd", "n")
...
base_plot <- function(summary_df, label_df, facet = TRUE, phase_name = NULL) {
  ...
  p <- ggplot(summary_df, aes(x = ambient_bin, y = mean, color = group, group = group)) +
    geom_line(linewidth = 0.70) +
    geom_point(size = 1.85) +
    geom_errorbar(aes(ymin = mean - sem, ymax = mean + sem), width = 0.16, linewidth = 0.50) +
    geom_text(
      data = label_df,
    ...
    scale_x_continuous(breaks = seq(21, 30, 1)) +
...
plot_data <- df[complete.cases(df[, c("subject", "group_cm", "phase", "final_clean_tewl", "ambient_temperature")]), ]
