...
output_xlsx <- file.path(tewl_dir, "TEWL_absolute_LMM_two_groups_baseline_exclude_PG.xlsx")
...
model_data <- model_data[complete.cases(model_data[, c("subject", "group_cm", "phase", "final_clean_tewl", "baseline_bdc_tewl")]), ]
...
names(group_phase_summary) <- c("group", "phase", "mean", "sd", "sem", "n")
...
names(baseline_summary) <- c("group", "baseline_mean", "baseline_sd", "baseline_sem", "n")

model_note <- data.frame(
  item = c("excluded_subjects", "groups", "outcome", "baseline", "model", "phase_definition", "stage_source", "data_level"),
  detail = c(
    "P and G excluded.",
    ...
    "final_clean_tewl ~ baseline_bdc_tewl + group_cm * phase + (1 | subject); model fitted on HDT-E/HDT-M/HDT-L only.",
