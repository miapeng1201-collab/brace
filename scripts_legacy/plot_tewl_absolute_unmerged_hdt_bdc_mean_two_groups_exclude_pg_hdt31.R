...
output_plot <- file.path(tewl_dir, "TEWL_absolute_BDC_mean_unmerged_HDT_two_groups_exclude_PG_exclude_HDT31_R.png")
output_xlsx <- file.path(tewl_dir, "TEWL_absolute_BDC_mean_unmerged_HDT_two_groups_exclude_PG_exclude_HDT31_summary.xlsx")

stage_levels <- c("BDC", "HDT1", "HDT7", "HDT13", "HDT25", "HDT37", "HDT43", "HDT49", "HDT55")
...
df$stage_plot <- NA_character_
df$stage_plot[df$stage_raw %in% c("BDC-12", "BDC-6")] <- "BDC"
df$stage_plot[df$stage_raw %in% c("HDT1", "HDT7", "HDT13", "HDT25", "HDT37", "HDT43", "HDT49", "HDT55")] <- df$stage_raw[df$stage_raw %in% c("HDT1", "HDT7", "HDT13", "HDT25", "HDT37", "HDT43", "HDT49", "HDT55")]
df$stage_plot <- factor(df$stage_plot, levels = stage_levels)

analysis_data <- df[complete.cases(df[, c("subject", "group_cm", "stage_plot", "final_clean_tewl")]), ]

subject_stage <- aggregate(
  final_clean_tewl ~ subject + group_cm + stage_plot,
  data = analysis_data,
  FUN = mean
)
...
summary_df <- aggregate(
  final_clean_tewl ~ group_cm + stage_plot,
  data = subject_stage,
...
p <- ggplot() +
  geom_line(
    data = subject_stage,
    aes(x = stage_plot, y = final_clean_tewl, group = interaction(subject, group_cm), color = group_cm),
    alpha = 0.16,
    linewidth = 0.32
  ) +
  geom_point(
    data = subject_stage,
    aes(x = stage_plot, y = final_clean_tewl, color = group_cm),
    alpha = 0.32,
    size = 1.25,
...
ggsave(output_plot, p, width = 8.4, height = 4.6, dpi = 320, bg = "white")

model_note <- data.frame(
  item = c("excluded_subjects", "excluded_stage", "groups", "stage_definition", "stage_source", "data_level"),
  detail = c(
    "P and G excluded.",
    "HDT31 excluded.",
    "Control vs Countermeasure; Countermeasure = Ex + ExAG.",
    "BDC = subject mean of BDC-12 and BDC-6; HDT stages remain unmerged.",
