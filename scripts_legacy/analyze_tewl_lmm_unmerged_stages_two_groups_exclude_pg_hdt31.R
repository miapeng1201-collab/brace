...
output_xlsx <- file.path(tewl_dir, "TEWL_LMM_unmerged_stages_two_groups_exclude_PG_exclude_HDT31.xlsx")
...
hdt_data <- hdt_data[complete.cases(hdt_data[, c(
  "subject", "group_cm", "stage", "final_clean_tewl", "baseline_bdc_tewl", "delta_percent_from_BDC"
)]), ]
...
absolute_model <- lmer(
  final_clean_tewl ~ baseline_bdc_tewl + group_cm * stage + (1 | subject),
  data = hdt_data,
  REML = FALSE
)

delta_model <- lmer(
  delta_percent_from_BDC ~ group_cm * stage + (1 | subject),
  data = hdt_data,
  REML = FALSE
)

res_abs <- extract_lmm(absolute_model, "Absolute_with_baseline_unmerged")
res_delta <- extract_lmm(delta_model, "Delta_unmerged")
emm_abs_pairs <- extract_emmeans_pairs(absolute_model, "Absolute_with_baseline_unmerged")
emm_delta_pairs <- extract_emmeans_pairs(delta_model, "Delta_unmerged")
...
model_note <- data.frame(
  item = c("excluded_subjects", "excluded_stage", "groups", "models", "baseline", "delta_formula", "stage_source", "data_level"),
  detail = c(
    "P and G excluded.",
    "HDT31 excluded.",
    "Control vs Countermeasure; Countermeasure = Ex + ExAG.",
    "Absolute model: final_clean_tewl ~ baseline_bdc_tewl + group_cm * stage + (1 | subject). Delta model: delta_percent_from_BDC ~ group_cm * stage + (1 | subject). Models fitted on unmerged HDT stages only.",
