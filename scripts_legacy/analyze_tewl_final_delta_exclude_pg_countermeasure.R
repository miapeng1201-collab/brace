...
output_xlsx <- file.path(tewl_dir, "TEWL_final_delta_percent_exclude_PG_countermeasure.xlsx")
output_plot <- file.path(tewl_dir, "TEWL_final_delta_percent_from_BDC_countermeasure_exclude_PG_R.png")
...
baseline <- stage_values[stage_values$stage_plot == "BDC", c("group_cm", "subject", "final_clean_tewl")]
...
fit_interaction <- lmer(delta_percent_from_BDC ~ group_cm * stage_plot + (1 | subject), data = delta_hdt, REML = FALSE)
fit_additive <- lmer(delta_percent_from_BDC ~ group_cm + stage_plot + (1 | subject), data = delta_hdt, REML = FALSE)
fit_no_group <- lmer(delta_percent_from_BDC ~ stage_plot + (1 | subject), data = delta_hdt, REML = FALSE)

model_tests <- data.frame(
  outcome = "final_clean_tewl_delta_percent_HDT_exclude_PG_countermeasure",
...
group_emmeans <- as.data.frame(emmeans(fit_additive, ~ group_cm))
pairwise_groups <- as.data.frame(pairs(emmeans(fit_additive, ~ group_cm)))

stage_group_means <- aggregate(
  delta_percent_from_BDC ~ group_cm + stage_plot,
  data = delta_hdt,
...
names(summary_df) <- c("group_cm", "stage_plot", "mean", "se", "n")

p <- ggplot() +
  geom_hline(yintercept = 0, color = "grey35", linewidth = 0.35) +
  geom_line(
    data = delta_values,
    aes(x = stage_plot, y = delta_percent_from_BDC, group = interaction(group_cm, subject), color = group_cm),
    alpha = 0.18,
    linewidth = 0.45,
    na.rm = TRUE
  ) +
  geom_point(
    data = delta_values,
    aes(x = stage_plot, y = delta_percent_from_BDC, color = group_cm),
    alpha = 0.28,
    size = 1.2,
    na.rm = TRUE
  ) +
...
model_note <- data.frame(
  item = c("excluded_subjects", "group_recode", "BDC_rule", "stages", "model"),
  detail = c(
    "P and G excluded.",
    "Ex and ExAG merged as Countermeasure.",
    "Subject BDC baseline is mean of available BDC-12 and BDC-6 final_clean_tewl.",
    paste(stage_order, collapse = ", "),
    "HDT stages only: delta_percent_from_BDC ~ group_cm * stage + (1 | subject); additive model used for group emmeans."
  ),
  stringsAsFactors = FALSE
)
