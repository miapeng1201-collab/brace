...
output_xlsx <- file.path(tewl_dir, "TEWL_skin_temperature_ambient_adjusted_LMM_absolute_delta.xlsx")
...
extract_emmeans_pairs <- function(model, model_name, group_col) {
  emm <- emmeans(model, as.formula(paste("~", group_col, "| stage_plot")))
...
summarize_values <- function(data, group_col, y_col) {
  out <- aggregate(
...
run_models <- function(model_source, group_col, group_levels, label) {
  data <- model_source[model_source$stage_plot %in% model_stage_levels, ]
  data <- data[complete.cases(data[, c(
    "subject", group_col, "stage_plot", "skin_adjusted",
    "baseline_bdc_skin_adjusted", "delta_percent_from_BDC"
  )]), ]
  data$subject <- factor(data$subject)
  data[[group_col]] <- factor(data[[group_col]], levels = group_levels)
  data$stage_plot <- factor(as.character(data$stage_plot), levels = model_stage_levels)

  abs_formula <- as.formula(paste(
    "skin_adjusted ~ baseline_bdc_skin_adjusted +",
    group_col, "* stage_plot + (1 | subject)"
  ))
  delta_formula <- as.formula(paste(
    "delta_percent_from_BDC ~",
    group_col, "* stage_plot + (1 | subject)"
  ))

  absolute_model <- lmer(abs_formula, data = data, REML = FALSE)
  delta_model <- lmer(delta_formula, data = data, REML = FALSE)

  abs_name <- paste0(label, "_Absolute_with_baseline")
  delta_name <- paste0(label, "_Delta_no_baseline")
  res_abs <- extract_lmm(absolute_model, abs_name)
  res_delta <- extract_lmm(delta_model, delta_name)

  list(
    anova = rbind(res_abs$anova, res_delta$anova),
    coefficients = rbind(res_abs$coefficients, res_delta$coefficients),
    fit = rbind(res_abs$fit, res_delta$fit),
    pairs = rbind(
      extract_emmeans_pairs(absolute_model, abs_name, group_col),
      extract_emmeans_pairs(delta_model, delta_name, group_col)
    ),
    model_data = data
  )
}
...
analysis_data <- df[complete.cases(df[, c("subject", "group", "group_cm", "stage_plot", "skin_adjusted")]), ]

subject_stage <- aggregate(
  skin_adjusted ~ subject + group + group_cm + stage_plot,
  data = analysis_data,
  FUN = mean
)
...
two <- run_models(model_source, "group_cm", c("Control", "Countermeasure"), "Two_group")

model_note <- data.frame(
  item = c("outcome", "subjects", "groups", "absolute_model", "delta_model", "baseline", "stages", "data_level"),
  detail = c(
    ...
    "Models fitted on all non-BDC stages: HDT1 to HDT55 plus R+1, R+6, R+12.",
...
write_xlsx(
  list(
    ...
    ANOVA_type3 = two$anova,
    Coefficients = two$coefficients,
    Model_fit = two$fit,
    EMM_group_pairs_by_stage = two$pairs,
    Two_group_absolute_summary = summarize_values(model_source, "group_cm", "skin_adjusted"),
    Two_group_delta_summary = summarize_values(model_source, "group_cm", "delta_percent_from_BDC"),
    Two_group_model_data = two$model_data,
