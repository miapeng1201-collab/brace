...
output_combined <- file.path(tewl_dir, "TEWL_vs_skin_temperature_binned_three_phases_three_groups_R.png")

phase_outputs <- c(
  BDC = file.path(tewl_dir, "TEWL_vs_skin_temperature_binned_BDC_three_groups_R.png"),
  `HDT1-19` = file.path(tewl_dir, "TEWL_vs_skin_temperature_binned_HDT1_19_three_groups_R.png"),
  `HDT37-55` = file.path(tewl_dir, "TEWL_vs_skin_temperature_binned_HDT37_55_three_groups_R.png")
)

colors_three <- c(Control = "#2F855A", Ex = "#D62728", ExAG = "#7B2CBF")
phase_levels <- c("BDC", "HDT1-19", "HDT37-55")
phase_labels <- c(BDC = "BDC", `HDT1-19` = "HDT1-19", `HDT37-55` = "HDT37-55")
...
fit_pvalues <- function(data) {
  data <- data[complete.cases(data[, c("subject", "final_clean_tewl", "skin_temperature", "group")]), ]
  data$subject <- factor(data$subject)
  data$group <- factor(data$group, levels = names(colors_three))

  out <- c(temp = NA_real_, group = NA_real_, temp_group = NA_real_)
  if (nrow(data) < 12 || length(unique(data$subject)) < 4 || length(unique(data$group)) < 2) {
    return(out)
  }
  ...
  out["group"] <- a[rownames(a) == "group", "Pr(>F)"]
  out["temp_group"] <- a[rownames(a) == "skin_temperature:group", "Pr(>F)"]
  out
}

make_summary <- function(data, bin_width = 0.5) {
  data$skin_bin <- round(data$skin_temperature / bin_width) * bin_width
  data <- data[complete.cases(data[, c("final_clean_tewl", "skin_bin", "group")]), ]

  summary_df <- aggregate(
    final_clean_tewl ~ phase + skin_bin + group,
    data = data,
  ...
  unpacked$group <- factor(unpacked$group, levels = names(colors_three))
  unpacked
}
...
base_plot <- function(summary_df, label_df, facet = TRUE, phase_name = NULL) {
  ...
  if (facet) {
    p <- p + facet_wrap(~ phase, nrow = 1, labeller = labeller(phase = phase_labels))
  } else {
    p <- p + ggtitle(phase_labels[[phase_name]]) +
...
plot_data <- df[complete.cases(df[, c("subject", "group", "phase", "final_clean_tewl", "skin_temperature")]), ]
