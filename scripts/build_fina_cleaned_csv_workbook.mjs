...
const inputJson = path.join(root, "Generated outputs", "finapress", "fina_cleaned_csv_payload.json");
const outputDir = path.join(root, "Processed data", "03 - FINAPRESS");
const outputPath = path.join(outputDir, "Fina_cleaned_processed.xlsx");
...
function sheetMatrix(records) {
  ...
  const rows = records.map((row) => headers.map((header) => (header === "date_iso" ? toDate(row[header]) : row[header] ?? null)));
  return [headers, ...rows];
}
...
function styleSheet(sheet, rowCount, colCount) {
  ...
  const sizingRange = rowCount > 1000 ? sheet.getRange(`A1:${lastCol}200`) : used;
...
function setNumberFormats(sheet, headers, rowCount) {
  ...
  headers.forEach((header, index) => {
    ...
    if (header === "date_iso") range.setNumberFormat("yyyy-mm-dd");
    if (["day", "day_real", "metric_present_n", "missing_metric_n", "n", "missing", "rows", "subjects", "measurements_present", "measurements_missing"].includes(header)) {
      range.setNumberFormat("#,##0");
    }
    if (header === "present_percent") range.setNumberFormat("0.0%");
    if (!["subject", "group", "stage", "stage_clean", "phase", "date_iso", "measurement_present", "metric", "field", "value"].includes(header)) {
      range.setNumberFormat(header === "present_percent" ? "0.0%" : "0.00");
    }
  });
}
...
for (const sheetName of ["README", "Clean_All", "Complete_Cases", "Missing_Rows", "Metric_Summary", "Coverage"]) {
...
readme.getRange("A1:B7").format.wrapText = true;

await fs.mkdir(outputDir, { recursive: true });

const preview = await workbook.render({ sheetName: "README", autoCrop: "all", scale: 1, format: "png" });
await fs.writeFile(
  path.join(root, "Generated outputs", "finapress", "fina_cleaned_readme_preview.png"),
