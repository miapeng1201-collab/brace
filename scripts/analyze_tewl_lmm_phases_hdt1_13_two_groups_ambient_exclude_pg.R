...
output_xlsx <- file.path(tewl_dir, "TEWL_LMM_BDC_HDTE1_13_HDTM_HDTL_two_groups_ambient_exclude_PG.xlsx")
...
hdt_data <- hdt_data[complete.cases(hdt_data[, c(
  "subject", "group_cm", "phase", "final_clean_tewl", "delta_percent_from_BDC", "ambient_temperature", "baseline_bdc_tewl"
)]), ]
...
res_abs <- extract_lmm(absolute_model, "Absolute_with_baseline_ambient")
res_delta <- extract_lmm(delta_model, "Delta_with_ambient")
emm_abs_pairs <- extract_emmeans_pairs(absolute_model, "Absolute_with_baseline_ambient")
emm_delta_pairs <- extract_emmeans_pairs(delta_model, "Delta_with_ambient")
...
model_note <- data.frame(
  item = c("excluded_subjects", "groups", "models", "delta_formula", "ambient_covariate", "phase_definition", "stage_source", "data_level"),
  detail = c(
    "P and G excluded.",
    "Control vs Countermeasure; Countermeasure = Ex + ExAG.",
    "Absolute model: final_clean_tewl ~ baseline_bdc_tewl + ambient_temperature + group_cm * phase + (1 | subject). Delta model: delta_percent_from_BDC ~ ambient_temperature + group_cm * phase + (1 | subject). Models fitted on HDT-E/HDT-M/HDT-L only.",
    ...
    "BDC = BDC-12 and BDC-6; HDT-E = HDT1, HDT7, HDT13; HDT-M = HDT25, HDT31, HDT37; HDT-L = HDT43, HDT49, HDT55.",
